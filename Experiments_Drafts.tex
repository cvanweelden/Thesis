...
\begin{align}
d_{\text{H}}(\vec{x}_i, \vec{x}_j) &= \|\vec{a}_i - \vec{a}_j\|_{\text{H}}, \\
...
\begin{align}
d_{\text{P}}(\vec{x}_i, \vec{x}_j) &= L(\vec{y}, \vec{y}^\prime) = 1 - \frac{\sum_{i,j} \left [ y_{i,j} = y^\prime_{i,j}\right]}{\sum_{i,j} \left [y_{i,j}, y^\prime_{i,j} \in \mathcal{L}\right ]}
\end{align}

We quantify the alignment between the learned metric and the target metric in the form of a squared prediction error, namely the mean squared error 
