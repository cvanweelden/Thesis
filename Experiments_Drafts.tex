...
%evaluate metric alignment method, need datasets with a target metric, developed for structered prediction problems where the loss function is a target metric
In order to evaluate the metric alignment method we use two datasets from structured prediction problems. The first dataset comes from an attribute-based classification problem, while the second comes from a semantic segmentation problem. Both these datasets have the property that there is a well-defined real-valued loss defined on the structured output space. This property allows us to test the idea of aligning the feature space to the output space through metric alignment. 

%have to formulate a learning task that tests the metric learning component seperately and the improvement of prediction with metric learning, we present two datasets that can test both these aspects
There are two things that we want to test with these datasets. Firstly, how well does the metric learned method described in this thesis approximate the target metric. In other words, how well does the method align feature space to the structured output space? This can be measured by the \acf{MSE} between the learned distances in feature space and the loss in output space for the points in the test set. The second result we want to evaluate is how much the learned metric improves performance over the default Euclidean metric on the overall classification/segmentation task. We do this by applying a \ac{kNN} classifier adapted to solve these structured problems and performance is measured by a task-specific loss function.

\section{Datasets}
%datasets, learning problems, features, ground-truth, loss functions, classes, number of instances, training/test split


\subsection{Attribute-based classification dataset}

%attribute based classification is an approach to classification in which classes are described by high-level "semantic" attributes, zero shot task, classes uniquely identified by their attributes
...
%predicting the attributes can be seen as a simple structured prediction problem, with Hamming distance as a loss function on binary attribute vectors
...
%we propose two learning task on this dataset, the full zero-shot learning task for which the dataset was originally presented and a simpler metric learning task in which the objective is to correctly predict the hamming distance between attributes of two input images
...
%semantic segmentation is the problem of predicting a per-pixel labeling of an image, where the labels are taken from a fixed set of object classes
Semantic segmentation is a computer vision problem in which the objective is to segment input images into contiguous regions by assigning labels to each pixel from a predefined set of class labels. 

%patches are extracted that are each described by a single (color)SIFT vector, visually similar patches are close together in feature space, we want semantically similar patches to be close together in feature space, semantic difference is measured by segmentation loss function, we train with segmenation loss function as target metric, learns small distance for large semantic similarity

%for our metric learning evaluation, we look at a per patch problem, we compare distances under the learned metric with segmentation loss used as the target metric

%for the full learning problem we use a kNN classifier on the patches extracted from the input image, the neighbor patches are then combined into a single segmentation, we see if this improves after metric learning



%\section{Experiments}
%description of experiments, evaluation measures, baselines, parameter settings, conversion of loss to target metric, sampling of constraints, number of repeats with random splits

%for our experiments we run metric alignment on the datasets described above and compare this against a baseline euclidean measure

%we also compare against existing methods, ITML we have to created binary similarity judgements, we do this by thresholding the target metric at its median value

%parameters for the methods are set by 2-fold cross-validation on the training set


\section{Attribute-based classification experiment}

\subsection{Feature extraction}

\subsection{Adapted \ac{kNN} classifier}

\subsection{Parameter settings}

\subsection{Evaluation measures}

\section{Semantic segmentation experiment}
...
%alignment of loss function and feature distance (pearson R statistic, equal axis plot), does the space get more structured? (clustering?)

%variations based on regularizer vs non-regularized

%different choices of loss function as target metric and correlation between input loss and minimized target metric (e.g. if target metric is intersection/union does i/u improve more than if target metric is pixel count)

%choice of constraint sampling and locality of metric, does it pull together similar patches or push appart dissimilar patches
